...
/**
 * lib/ must match what `tsc -b` used to emit from this config, so take the
 * target from it instead of hardcoding one; core targets esnext while every
 * other package targets ES2018.
 */
/**
 * lib/ JavaScript must match the target tsc would have compiled to, so read it
 * from the package tsconfig instead of hardcoding one.
 */
const tsconfig = 'tsconfig.json';
const project = ts.getParsedCommandLineOfConfigFile(tsconfig, {}, {
  ...ts.sys,
  onUnRecoverableConfigFileDiagnostic: (diagnostic) => {
    throw new Error(ts.flattenDiagnosticMessageText(diagnostic.messageText, '\n'));
  },
})!;
const libTarget = ts.ScriptTarget[project.options.target ?? ts.ScriptTarget.ES2018]
  .toLowerCase()
  .replace('latest', 'esnext');
...
const common = {
  ...
  outDir: 'dist',
  platform: 'browser',
  // esbuild had no target flag, so the bundles were esnext; lib/ overrides this.
  target: 'esnext',
  sourcemap: true,
  dts: false,
  clean: false,
