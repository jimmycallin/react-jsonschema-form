...
- `build:ts` now clears `tsconfig.tsbuildinfo` as well as `lib/`. With the config at the package root the build-info file lands there rather than inside `lib/`, and deleting only `lib/` would leave `tsc -b` believing the project was up to date and emitting nothing
...
- Added `pnpm run verify-entrypoints`, which loads every published package's public entrypoint with Node as both ESM and CommonJS, and wired it into CI after the build. `pnpm run build` only proves the files were produced; it does not prove Node can resolve what they import, and Node's ESM resolver has no extension search or directory index lookup. Entrypoints that cannot load because of a third-party package are listed with their reason and do not fail the check, and the check also fails if one of those starts working, so the list cannot go stale
...
- Declared `"sideEffects": false` in `package.json`, allowing bundlers to tree-shake unused exports
- Fixed `BaseInputTemplate`'s `ui:allowClearTextInputs` clear button to store `ui:emptyValue` (`undefined` by default) instead of always storing `""`, so clicking it now behaves exactly like clearing the input by typing. **Potentially breaking change:** an app that never sets `ui:emptyValue` and clicks the clear button on a field (rather than backspacing it) previously got `""` back; it now gets `undefined`, matching every other way of emptying the field.
- Fixed `lib/index.js`, which Node could not load at all. The source imported `react-bootstrap/Col` and `@react-icons/all-files/bs/BsPlus` — a directory import and an extensionless import, neither of which Node's ESM resolver supports — so every ESM consumer that did not go through a bundler failed on `ERR_UNSUPPORTED_DIR_IMPORT`/`ERR_MODULE_NOT_FOUND`. Components now come from the `react-bootstrap` entry point and icons name their file explicitly. `RangeWidget` uses `Form.Range`, since `FormRange` is not exported from the package root
