...
- Reworked how the test suites run: the whole monorepo now runs through vitest's native `projects` mode from the repo root (one `pnpm test` command and process instead of one vitest process per package via nx), shared vitest defaults live in `testing/vitest.base.ts` with package configs only declaring their deltas, the 100%-coverage roster for `@rjsf/utils` and the validators is single-sourced there, and `@rjsf/utils` tests default to the faster `node` environment with only the DOM-dependent files opting into jsdom
