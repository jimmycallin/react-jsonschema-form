...
Review revision: 2026-09-11. The milestone retains strict ownership, now with guarded controlled default proposals, errors-only controlled reset, and lossless operation serialization. Reset baselines/options, new setters/error-clearing methods, a validation observer, and synchronous pending-value reads remain deferred. These design records live in `rfcs/` because root `docs/` is deprecated.

## Conclusion

Keep the strict parent-owned versus form-owned distinction and retain the class for the behavioral migration. Keep existing public names and domain behavior where compatible with ownership. The research establishes useful precedents, but does not require adopting them all at once. Mode handling, a supported getter/ref interface, and explicit default-proposal and controlled-reset rules belong in the first milestone. Broader default algorithms, baseline APIs, and event redesigns remain separate.
...
| [Mantine uncontrolled mode](https://mantine.dev/form/uncontrolled/)                                                                                  | The mode selects state-backed versus ref-backed form storage and changes how inputs receive values/defaults. It recommends uncontrolled mode for performance.                                                                                                             | This use of “controlled” is about the form-to-input boundary. Our `dataMode` is about who owns the entire JSON value and does not promise Mantine's performance characteristics.             |
...
Use defined-value inference at mount, plus optional `dataMode='controlled'` for a root that starts undefined. Freeze the chosen mode. Null remains a JSON value. A controlled value can subsequently become undefined without handing ownership away. Use development warnings with deterministic precedence for conflicting props or attempted mode changes.

The explicit mode is a domain-specific choice, not a React requirement. Alternatives were surprising own-property detection, forbidding controlled undefined, or adding separate exported form components. Keep current component/ref structure and ordinary optional-prop forwarding.
...
Keep mode-change and conflicting-prop diagnostics, and add the small development mount warning for controlled data without an accepting handler unless readonly/disabled. It catches a common migration mistake; refusing it to avoid a diagnostics framework was disproportionate. It cannot detect a logging-only handler, and it does not determine ownership.

The explicit `dataMode` escape hatch remains useful for a controlled root starting undefined. Additive access/type work and private mode resolution can land first, but activating frozen ownership is behavioral. Do not publish a `dataMode` prop that is accepted yet ignored, or diagnostics describing strict behavior while the old hybrid remains active.
