...
Review revision: 2026-09-15. The milestone retains strict ownership, now inferred from `formData` with no explicit mode prop, plus parent-seeded controlled defaults, errors-only controlled reset, and lossless operation serialization. This revision adds a survey of imperative read APIs and withdraws the controlled defaults proposal on its evidence. Reset baselines/options, new setters/error-clearing methods, a validation observer, and synchronous pending-value reads remain deferred. These design records live in `rfcs/` because root `docs/` is deprecated.

## Conclusion

Keep the strict parent-owned versus form-owned distinction and retain the class for the behavioral migration. Keep existing public names and domain behavior where compatible with ownership. The research establishes useful precedents, but does not require adopting them all at once. Inferred mode handling with no new ownership prop, a supported getter/ref interface, parent-owned controlled defaults, and explicit controlled-reset rules belong in the first milestone. Broader default algorithms, baseline APIs, and event redesigns remain separate.
...
| [React Hook Form: getValues](https://react-hook-form.com/docs/useform/getvalues)                                                                     | A synchronous read of current values that does not subscribe or rerender. A value set with `setValue` is readable immediately because both touch one mutable store.                                                                                                       | The getter shape is standard. Its read-your-writes guarantee comes from owning a store, which RJSF does not; see the imperative-reads survey.                                             |
...
| [Mantine form.getValues](https://mantine.dev/form/uncontrolled/)                                                                                     | Added with uncontrolled mode: "`form.getValues()` always returns the latest values while `form.values` is outdated in uncontrolled mode." Returns a ref value, so it cannot be an effect dependency.                                                                      | Direct precedent for `getFormData()` existing for uncontrolled mode specifically, arrived at independently.                                                                               |
...
| [Ant Design Form](https://ant.design/components/form/)                                                                                               | The form manages field data, uses `initialValues` and imperative setters, and deliberately does not emit its user change callbacks for programmatic setters. Methods live on a `Form.useForm()` instance passed in as a prop, not on a component ref.                     | Callback semantics vary across libraries. RJSF can keep imperative change proposals, but must document them. The hook-instance shape is the closest component-based precedent; see below. |
...
| [JSON Forms React integration](https://jsonforms.io/docs/integrations/react/) and [FAQ](https://jsonforms.io/faq/)                                   | Change notifications include validation and an initial notification; the FAQ describes feedback-loop risks when recreating data in the callback.                                                                                                                          | The initial event reports validation, not a data proposal. It is not precedent for a mount-time data callback, and the FAQ's feedback-loop warning is evidence against one.               |
...
### Imperative reads: what the survey shows

Every library that owns its value ships a whole-value getter. The question is not whether to have one but what shape and guarantees it carries.

| Library              | Read API                               | Attached to                                    | Read-your-writes               |
| -------------------- | -------------------------------------- | ---------------------------------------------- | ------------------------------ |
| React Hook Form      | `getValues()`                          | `useForm()` return                             | Yes, synchronous mutable store |
| Mantine              | `form.getValues()`                     | `useForm()` instance                           | Yes, ref-backed                |
| Final Form           | `form.getState().values`               | `FormApi` instance                             | Yes                            |
| Ant Design           | `form.getFieldsValue()`                | `Form.useForm()` instance, passed in as a prop | Yes, own store                 |
| TanStack Form        | `form.state.values`, `getFieldValue()` | `FormApi` instance                             | Yes                            |
| Formik               | `ref.current.values`                   | component ref via `innerRef`                   | No, React state                |
| JSON Forms, uniforms | none                                   | caller owns the value                          | not applicable                 |

Three conclusions follow.

The getter is justified by uncontrolled mode alone. Mantine added `getValues()` for exactly that reason. The caller-owns-value camp, which is RJSF's controlled mode, has no getter because there is nothing to get; the controlled half of `getFormData()` is handle symmetry, not a feature.

The read semantics diverge, not the existence. Six of seven getters are read-your-writes because they read a mutable store, not React state. RJSF's committed-read semantics are weaker than every one of them, and the failure mode for a developer arriving from those libraries is a silently stale value after `setFieldValue()`. The specification keeps committed semantics because the alternative is a store, and requires the caveat in the method's own TSDoc.

The ref shape is correct for a non-store component. Six libraries expose methods on a hook-created instance, but in each the instance is the store and the component is a renderer bound to it. RJSF's controlled mode has no store to hold; the parent's state is the store. A `ref` carrying a narrow handle is what `useImperativeHandle` exists for, and React 19's `ref`-as-prop removes the `forwardRef` friction. Adopting a hook-instance API would mean adopting a store, which is the third architecture above and a separate decision. Formik being the other ref user is a coincidence of age, not evidence against the shape.
...
### Implement now: controlled forms do not propose defaults

An earlier revision re-admitted a one-shot controlled defaults proposal on mount and schema change, reasoning that ownership constrains what is rendered but does not forbid proposing. That reasoning was valid and is withdrawn anyway, on three grounds.

No precedent. Not one surveyed library emits a data-changing callback on mount. Store-owning libraries install defaults internally, which is RJSF's uncontrolled mode and is unchanged. JSON Forms' initial event reports validation, and its own FAQ warns about feedback loops from recreating data in the callback. [JSON Forms React integration](https://jsonforms.io/docs/integrations/react/)

Against React guidance. `<input value>` never calls `onChange` unprompted. A parent doing `onChange={(e) => setData(e.formData)}` receives a mount-time `setState` from a child callback, which is state synchronization through an effect with one extra hop. [You Might Not Need an Effect](https://react.dev/learn/you-might-not-need-an-effect)

Against the milestone's own principle. Controlled means the parent owns the value, and the value includes its defaults. The proposal was the hybrid's ghost: the form still authoring the parent's data, only asking first. It also required the specification's single largest mechanism, loop prevention across rejection, transformation, replacement, equal schemas, StrictMode, and child effects, plus two of seven contract tests, all to police one convenience.

The replacement was already in the specification as an option: `createSchemaUtils(validator, schema, behavior).getDefaultFormState(schema, seed)` in the parent's `useState` initializer. Two lines, using options the parent already passes to `<Form>`, and it fixes first-render and server output, which a post-mount proposal never could. Uncontrolled mode keeps internal initialization from `initialFormData` exactly as today.
...
Reset baselines, new arguments, and dedicated clear-errors methods remain deferred.
...
Move uncontrolled default notifications out of the constructor into guarded post-commit delivery, retaining their existing condition. A controlled mount emits nothing.

### Preserve now; reconsider separately: defaults

Strict controlled ownership means no internal installation or proposal of generated data; the parent seeds defaults. Uncontrolled initialization stays internal.
...
A hook-created form instance in the style of `useForm()` is a separate question from the conversion, and a larger one: in every surveyed library that instance is the store. Deciding on that shape is deciding whether RJSF owns a store. The imperative-reads survey above explains why the `ref` handle is the right fit until that decision is made.
...
The implementation specification now separates six design-risk contract tests from a map of existing regression coverage. Earlier numbered matrices are superseded; this reduces ceremony without removing coverage. Deferred APIs remain opportunities, not pending implementation obligations.
...
The six focused contract tests gate this change, supplemented by existing domain regressions. Do not spend review attention duplicating tests just to satisfy a numbered matrix.
