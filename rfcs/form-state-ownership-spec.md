...
| Uncontrolled schema or default option change | Fields whose resolved schema is unchanged keep their count, which holds because `retrieveSchema()` and the render-context derivation share unchanged subtrees. |
...
| ---------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| **A: `Form.tsx`**, strictly serial | A1 rjsf-team#5297 (merged) → A2 render-context extraction → A3 `applyChange` extraction → A4 ownership switch with child rejection fixes | A2 + A3 in review as PR 2; A4 after gate 2             |
...
- [ ] Gate 2: A3, the tests PR (B, merged as rjsf-team#5300), the event type (C1, merged as rjsf-team#5298) and the child prep (C2, merged as rjsf-team#5299) merged before the ownership switch (A4) opened. Only PR 2 (A2 + A3, in review) is outstanding.
