...
Two pieces of this were considered for rjsf-team#5297 and deliberately left for PR 2. The default-state and validation caches need each step's explicit input set as their key, which only exists once the steps are extracted out of `getStateFromProps`; a cache written inside that function would be rewritten by the extraction. A compile-time check that every function-typed `FormProps` key appears in `IDENTITY_PROP_KEYS` would have fenced a list PR 2 turns into per-step cache keys and PR 3 strips of its `deepEquals` fallback. When PR 2 writes those keys, the check worth having is the one in section 6: a replaced `validator`, `customValidate`, `transformErrors`, template, widget or field takes effect, which fails whenever a step's key omits one of its inputs.
...
| ---------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| **A: `Form.tsx`**, strictly serial | A1 rjsf-team#5297 (merged) → A2 render-context extraction → A3 `applyChange` extraction and caches → A4 ownership switch with child rejection fixes | A2 now; A3 after A2; A4 after gate 2                 |
...
### A2: Render-context extraction (next; gate 1 cleared)
...
### A3: `applyChange` extraction and caches (after A2)

Extract shared edit processing and initialization into `applyChange(current, change, context)` as section 4 describes, ending in `replaceEqualDeep(current, next)`, with `setSharedState` becoming its commit. Add the last-call caches for the default-state and validation steps, keyed by their explicit input sets. Preserve useful queue behavior. The mode resolver planned here in an earlier revision was dropped: it is `props.formData !== undefined`, one line with no consumer until A4, so it lands there with its unit tests. Exit: existing behavior passes and the A4 diff is a removal of the reconciler plus the switch, not a refactor.

### A4, PR 3: Atomic ownership switch and necessary child fixes (after gate 2)
...
### D1, PR 4: Remaining documentation and examples (drafted during A4 review)
...
- [ ] Gate 2: A3, the tests PR (B, merged as rjsf-team#5300), the event type (C1, merged as rjsf-team#5298) and the child prep (C2, merged as rjsf-team#5299) merged before the ownership switch (A4) opened. Only A3 is outstanding.
