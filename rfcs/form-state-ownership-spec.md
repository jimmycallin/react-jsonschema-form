...
**What stays an effect, and writes nothing.** Two notifications to the parent, which React has no other place for: the seed-defaults `onChange` after mount (today `componentDidMount`, guarded against StrictMode's double mount), and the transition `onChange` when a schema or uiSchema change moved a self-owned form's rendered data (section 3.4), recognized by comparing the rendered data with the previous render's, which the memo holds anyway. Dropping the transition notification would remove the second effect; that is a section 3.4 decision, not a conversion one.
...
2. The conversion itself: `useState` for the owned state, `useMemo` for the derived, `useRef` for the queue and the mount guard, two effects for the notifications, `useImperativeHandle` for the handle, `Ref<FormHandle>` for the prop. rjsf-team#5310 has already moved the `Form` tests off the instance state, so the behavioral tests carry over unchanged.
