...
import type { CustomMergeAllOf, FormContextType, RJSFSchema, UiSchema, ValidatorType } from '../types.ts';
...
>(
  ...
  if (schema.items) {
    const itemsSchema = retrieveSchema<T, S, F>(validator, schema.items as S, rootSchema, undefined, customMergeAllOf);
