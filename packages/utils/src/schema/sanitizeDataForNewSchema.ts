...
>(
  ...
  if (newProperties) {
    ...
    keys.forEach((key) => {
      // `data` is only known to be an object here, so the child value is asserted to the field type `T`
      const formValue = (isObject(data) ? data[key] : undefined) as T | undefined;
